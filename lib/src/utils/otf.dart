...
bool checkBitMask({required int value, required int mask}) =>
    (value & mask) == mask;

int calculateTableChecksum({required ByteData encodedTable}) {
...
int calculateFontChecksum({required ByteData byteData}) {
  return (kChecksumMagicNumber - calculateTableChecksum(encodedTable: byteData))
      .toUnsigned(32);
}

int getPaddedTableSize({required int actualSize}) =>
    (actualSize / 4).ceil() * 4;
...
List<int> relToAbsCoordinates({required List<int> relCoordinates}) {
...
List<int> absToRelCoordinates({required List<int> absCoordinates}) {
...
class Revision {
  ...
  @override
  int get hashCode => combineHashCode(
        first: major.hashCode,
        second: minor.hashCode,
      );
